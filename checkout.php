...
<blockquote>
	<img src="lg.png">
	<input class="hi" style="float: right; margin: 2%;" type="button" name="cancel" value="Главная" onClick="window.location='index.php';" />
</blockquote>
</header>
<?php
session_start();
...
<?php
	echo '<h2 style="color: #000;">Подтверждение заказа</h2>';
	echo "<table style='width:100%'>";
	echo "<tr><th>Описание</th>";
	echo "<th></th></tr>";
	$row = $result->fetch_assoc();
	echo "<tr><td>Имя: </td><td>".$row['CustomerName']."</td></tr>";
	echo "<tr><td>Паспорт: </td><td>".$row['CustomerIC']."</td></tr>";
	echo "<tr><td>E-mail: </td><td>".$row['CustomerEmail']."</td></tr>";
	echo "<tr><td>Телефон: </td><td>".$row['CustomerPhone']."</td></tr>";
	echo "<tr><td>Пол: </td><td>".$row['CustomerGender']."</td></tr>";
	echo "<tr><td>Адрес: </td><td>".$row['CustomerAddress']."</td></tr>";
	echo "<tr><td>Дата: </td><td>".$row['DatePurchase']."</td></tr>";
	echo "</blockquote>";
...
if(!isset($_SESSION['id'])){
	...
	echo 'ИМЯ:<br><input type="text" name="name" placeholder="Full Name">';
	echo '<span class="error" style="color: red; font-size: 0.8em;"><?php echo $nameErr;?></span><br><br>';

	echo 'Паспорт:<br><input type="text" name="ic" placeholder="xxxxxx-xx-xxxx">';
	...
	echo 'Телефон:<br><input type="text" name="contact" placeholder="012-3456789">';
...
<?php
	echo '<input class="button" type="submit" name="submitButton" value="ПРОВЕРИТЬ">';
	echo '</form><br><br>';
}
